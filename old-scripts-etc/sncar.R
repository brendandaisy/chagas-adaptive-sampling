...
expand.grid(r = seq(-15, 15, length.out=20), dl = seq(10, 10, length.out=20)) %>%
  pmap_dfr(~{
    C
    tibble(rho = r, )
  })
for (r in seq(-10, 10, 1)) {
  print(paste0(r, ': ', is.positive.definite(diag(rowSums(dist_mat)) - r * dist_mat)))
}
